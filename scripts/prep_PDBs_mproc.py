...
def superimpose(listapdbs):
    ...
    for pdb in sorted(listapdbs):
        ...
        s_path = pdb.split("/")
        s_pdb = s_path[-1].split("_") 
        pdbname = s_pdb[0] 
        unipcode = s_pdb[2].upper() 
...
path_recWat = sys.argv[3]  # path where crystals with internal waters are located
path = sys.argv[4]  # path where the above folder is located
path_req = sys.argv[2] + sys.argv[5] + "/"
...
if len(folder_RW) > 10:
    ...
    with ProcessPoolExecutor(max_workers=num_cpus) as executor:
        executor.map(superimpose, div_pathsRW)
