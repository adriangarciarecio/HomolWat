...
def get_wats_filtered(letter, pdb_id, recep, filename):
    wats_all, wats_good, sodiums = [], [], []
    f_in = open(path_rw + filename + ".pdb", "r")
    f_r = f_in.readlines()
...
for receptor in Recs_resol:
    ...
    for elem in pdbs:
        ...
        for repli in ["A", "B", "C", "D"]:
            repli_let = "_" + repli + "_"
            try:
                match = elem + repli_let + rec.lower()
                rw_files = os.listdir(path_rw)
                # Use regex to match files based on the provided pattern
                file_pattern = fr'^{match}(?!.*wat).*\.pdb$'
                pattern = re.compile(file_pattern)
                matching_files = [file for file in rw_files if pattern.search(file)]
                pdbID = matching_files[0][:-4]
            except:
                continue
            if file_exist(path_rw + pdbID + ".pdb"):
                ...
                wat_ions = get_wats_filtered(repli_let, elem, rec, pdbID)
